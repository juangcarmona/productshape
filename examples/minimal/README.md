...
Layout mirrors a consumer repository's `docs/product/model/`, with `product/` standing in for `docs/product`.

## Running the CLI against this example

The example carries its own `.product/config.yaml` (pointing `product-root` at `product/`), so it is a self-contained product repository. Validate it directly, without leaving the monorepo root:
