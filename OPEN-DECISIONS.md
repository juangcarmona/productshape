...
then lives in the one artifact that enforces it: a product constraint for product decisions, the
code and configuration for implementation decisions, or the Git history for the reasoning. It is
never duplicated back into this file, so this list stays short and true to its name. Entry IDs
are stable and are not renumbered when an entry is removed; other documents may reference them.
