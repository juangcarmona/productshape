...
export const PDAC_SPEC_TEMPLATE_BLOCK = block([
  ...
  '     it were new.',
  '  3. Declare the scope of this document, or it stays unclassified and verification fails.',
  '     A declaration is read only from an HTML comment of its own carrying a `pdac-scope` key,',
  '     or from a `pdac-scope` key in YAML frontmatter; Spec Kit documents have no frontmatter,',
  '     so write the comment. A cited document declares the value `cited` and carries at least',
  '     one citation: citations alone never bind, and the declaration alone never binds either.',
  '     If instead this feature has no product-semantic dependency, a human replaces this',
  '     section with a reasoned exemption declaring the value `none` with a non-empty reason.',
  '     Both exact forms are in .specify/memory/pdac.md. Never declare the exemption just',
...
export const PDAC_PLAN_TEMPLATE_BLOCK = block([
  ...
  '  planning against a stale citation builds on meaning that has already moved. Declare the',
  '  scope of this document too, or it stays unclassified and verification fails: a declaration',
  '  is read only from an HTML comment of its own carrying a `pdac-scope` key, or from a',
  '  `pdac-scope` key in YAML frontmatter, and Spec Kit documents have no frontmatter. A cited',
  '  plan declares the value `cited` and carries at least one citation. If this plan has no',
  '  product-semantic dependency, a human replaces this section with a reasoned exemption',
  '  declaring the value `none` with a non-empty reason. Both exact forms are in',
...
export const PDAC_TASKS_TEMPLATE_BLOCK = block([
  ...
  '  citation to every artifact it depends on (emit with `npx prodshape cite --id <ID> --digest',
  "  <digest>`). Staleness follows the cited artifact's own digest, never its relationships: a",
  "  task that depends on a business rule's parameter cites the rule itself, not only the",
  '  requirement derived from it. A task that changes cited behaviour includes a follow-up task',
  '  to refresh the affected citations. Never infer implementation, verification, release or',
  '  deployment from Product Change status. Declare the scope of this document too, or it stays',
  '  unclassified and verification fails: a declaration is read only from an HTML comment of its',
  '  own carrying a `pdac-scope` key, or from a `pdac-scope` key in YAML frontmatter, and Spec',
  '  Kit documents have no frontmatter. A cited task list declares the value `cited` and carries',
  '  at least one citation. If this task list has no product-semantic dependency, a human',
  '  replaces this section with a reasoned exemption declaring the value `none` with a non-empty',
  '  reason. Both exact forms are in .specify/memory/pdac.md.',
  '-->',
  '',
