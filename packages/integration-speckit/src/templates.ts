...
export const PDAC_TASKS_TEMPLATE_BLOCK = block([
  ...
  '  Change status. Declare the scope of this document too, or it stays unclassified and',
  '  verification fails: a declaration is read only from an HTML comment of its own carrying a',
  '  `pdac-scope` key, or from a `pdac-scope` key in YAML frontmatter, and Spec Kit documents',
  '  have no frontmatter. A cited task list declares the value `cited` and carries at least one',
  '  citation. If this task list has no product-semantic dependency, a human replaces this',
  '  section with a reasoned exemption declaring the value `none` with a non-empty reason. Both',
  '  exact forms are in .specify/memory/pdac.md.',
  '-->',
  '',
