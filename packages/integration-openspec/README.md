...
```text
/opsx:new <change> --schema product-change
        |
/opsx:continue (or /opsx:ff)      authors proposal.md, then product-change/change.md + product-change/proposed/**
        |
...
```text
node openspec/schemas/product-change/scripts/product-apply.mjs --change <name> [--dry-run]
```
...
import {
  ...
  listOpenSpecProductChanges, // OpenSpec changes hosting product-change/change.md
