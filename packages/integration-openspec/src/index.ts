...
Mapping a change's intent to its impacted artifacts is semantic work: compare the intent against the entire product definition first, then expand the identified set structurally with \`npx prodshape impact <ID>\`. When the intent contradicts or exceeds the accepted definition, record that drift in the proposal as an explicit warning for humans to resolve — never resolve it silently.
...
export const PDAC_RULES: Record<string, string[]> = {
  proposal: [
    ...
    "If this change's goals contradict the accepted product definition, or require behaviour it does not carry, that is product-definition drift: record it in the proposal as an explicit warning under a 'Product definition drift' note naming the artifacts involved, and leave the resolution to humans — the developer and the product owner agree whether to propose a Product Change or adapt this change. Never resolve drift silently, drop or weaken a citation to hide it, or paraphrase around the conflict.",
