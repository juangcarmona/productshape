...
Mapping a change's intent to its impacted artifacts is semantic work: compare the intent against the entire product definition first, then expand the identified set structurally with \`npx prodshape impact <ID>\`. When the intent contradicts or exceeds the accepted definition, record that drift in the proposal as an explicit warning carrying the marker \`<!-- pdac-drift ids="..." summary="..." -->\` (enumerable with \`npx prodshape drift\`) for humans to resolve — never resolve it silently.
...
export const PDAC_RULES: Record<string, string[]> = {
  proposal: [
    ...
    "Identifying the impacted artifacts is a dedicated step before proposal content is written, and it is semantic first: read the entire product definition under the model root (docs/product/model by default), compare this change's motivating intent and goals — the backlog item that started it, if one exists — against it, and identify every artifact this change depends on, alters or contradicts. Then expand the set structurally: run `npx prodshape impact <ID>` on each identified artifact and walk the reported direct and transitive neighbours. Record the impacted set in the proposal, cite every impacted artifact from each document that derives from it (proposal, specs, design, tasks), and list any examined-but-excluded neighbour so the exclusion is visible, never silent.",
    'If this change\'s goals contradict the accepted product definition, or require behaviour it does not carry, that is product-definition drift: record it in the proposal as an explicit warning under a \'Product definition drift\' note naming the artifacts involved, carried by the marker `<!-- pdac-drift ids="<ID>[, <ID>...]" summary="<one line>" -->` on a line of its own so `npx prodshape drift` can enumerate it, and leave the resolution to humans — the developer and the product owner agree whether to propose a Product Change or adapt this change. Never resolve drift silently, drop or weaken a citation to hide it, or paraphrase around the conflict.',
