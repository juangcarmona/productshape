...
Every current OpenSpec document (proposal, specs, design, tasks and openspec/specs/) has exactly one scope state: bound (it carries at least one citation, or declares \`pdac-scope: cited\`), exempt (a human declared \`pdac-scope: none\` because the document has no product-semantic dependency), or unclassified (neither declared — a verification failure). Binding and exemption are human declarations: never declare \`pdac-scope: none\` merely because citations are missing, and a \`pdac-scope: cited\` document with zero citations fails. Verify with \`npx prodshape citations verify --provider openspec\`: it enumerates the expected documents, so zero discovered citations is a set of failures, never a pass. The scope gate applies to current documents only; citations in archived changes are still checked, with problems reported as warnings, because archived history cannot be edited.
