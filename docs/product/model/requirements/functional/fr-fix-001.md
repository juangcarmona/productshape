...
Refusing rather than partially applying follows from what is being modified. These are canonical files, and their identifiers are referenced from other artifacts, changes, slices and handoffs; a half-completed rename leaves the model referencing artifacts that are no longer where anything expects them. Refusing the whole repair keeps a recoverable state recoverable.
