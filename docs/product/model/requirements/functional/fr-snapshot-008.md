...
verification:
  ...
  - scenario: Search, filtering and browsing remain responsive at the reference model scale
...
Search, filtering and browsing MUST remain responsive at the reference model scale that QR-SCALABILITY-001 defines.
...
At model scale, discovery is the experience: a reader who cannot find an artifact cannot read it, and a reader who loses their query every time they open a result will stop opening results. The Catalog is where "every artifact is reachable" becomes a practical claim rather than a structural one — reachable in a few keystrokes, not reachable in principle.
...
- The same interactions are exercised on the reference model and remain responsive, measured as QR-SCALABILITY-001 requires.
