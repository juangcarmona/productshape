...
outputs MUST be byte-identical across runs and across platforms, diagnostics MUST appear in a
deterministic order (by file, then code, then target), and impact analysis and handoff status MUST
return the same answers for the same content. Results MUST be independent of the checkout's line
endings, path separators, file discovery order and any other environmental accident.
...
Automated determinism tests run validation, impact analysis and handoff status twice in one
session and once per supported platform over committed fixtures, asserting equality of outputs
against each other and against committed golden files. A dedicated fixture is checked out with
CRLF endings to prove digest and output stability. These tests are part of the release gate, so
no version ships with a nondeterministic operation.
