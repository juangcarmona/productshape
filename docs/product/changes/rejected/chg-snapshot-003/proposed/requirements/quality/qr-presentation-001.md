...
verification:
  ...
  - scenario: A map node presents its human-readable title as its primary identity whenever it is shown at readable detail, with kind and identifier secondary
  - scenario: Artifact-kind colour is an accent on a node rather than a large filled surface
  - scenario: Focused, selected, related, background and hovered nodes are each visually distinct, and distinguishable without colour
