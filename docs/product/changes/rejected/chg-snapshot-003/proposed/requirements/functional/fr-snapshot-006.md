...
title: Hold one selected artifact, one band scope, and address every view
status: active
derived-from:
  - UC-SNAPSHOT-EXPLORE-001
  - BR-IDENTITY-001
verification:
  - scenario: Selecting an artifact from the map, a search result, the artifact browser or a relationship link performs one identical operation
  - scenario: That operation sets the selected artifact, updates the inspector, updates the address, and centres and fits the neighbourhood
  - scenario: Hovering an artifact may preview it but changes neither the selection nor the focus
  - scenario: Band scope is represented in the address separately from the selected artifact, and selecting a band changes neither the selection nor the focus
...
The Product Snapshot MUST hold exactly one selected artifact at a time, shared by every part of the page.

**Selection is one operation.** Selecting an artifact from the map, from a search result, from the artifact
browser, or from a declared or derived relationship link MUST perform the identical operation: set the
selected artifact, update the inspector to show it, update the address to reflect it, and centre and fit its
neighbourhood on the map. No surface may perform a partial version of this — moving the selection without
showing it, or showing it without moving the map — and every surface displaying the artifact MUST reflect
that it is selected.

**Hover is not selection.** Hovering or otherwise previewing an artifact MAY reveal information about it and
MUST NOT change the selected artifact, the focus or the address.

**Band scope is separate state.** Selecting a band MUST scope or highlight the landscape and the artifact
browser. It MUST NOT select an artifact and MUST NOT create or clear a focused neighbourhood. Band scope
MUST be represented in the addressable state separately from the selected artifact, so a scoped landscape is
directly linkable and is restored by Back and Forward independently of the selection.
...
The addressable state MUST represent at least the active view, the selected artifact's identifier, the map
state, and the band scope. Addressing MUST use the URL fragment, so that it works identically when
...
- The reader selects an artifact from the rail, then finds another through search, then follows a
  relationship link, then selects a node on the map. At every step the same four things happen: the
  selection is set, the inspector shows it, the address changes, and the map centres and fits its
  neighbourhood. Exactly one artifact is selected and every surface agrees.
- The reader hovers several artifacts on the map without clicking. The selection, the focus and the address
  are unchanged throughout.
- The reader selects a band. The landscape and the artifact browser narrow to it, no artifact becomes
  selected, no focus is created, and the address records the scope separately from any selected artifact —
  so a scoped landscape can be linked, and clearing the scope leaves the selection intact.
