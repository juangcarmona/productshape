---
id: FR-DISTRIBUTION-001
type: functional-requirement
title: Generate AI provider integrations from canonical assets
status: active
derived-from:
  - UC-INIT-001
  - BR-AI-001
  - BR-MUTATION-001
verification:
  - scenario: Installed provider files carry managed-file headers with version metadata
  - scenario: Regenerating for the same provider, version and configuration reproduces identical files
  - scenario: A hand-edited managed file is detected and reported
  - scenario: A managed file the current configuration no longer generates is removed
  - scenario: A managed file that would be removed but was hand-edited is kept and reported
  - scenario: Installation and removal can be reported without performing them, and the report matches what performing them produces
  - scenario: Removing an integration preserves and reports a hand-edited managed file, and keeps its record
  - scenario: An explicit destructive request removes a hand-edited managed file and reports what it removed
  - scenario: An installation record that exists but cannot be trusted stops the operation instead of reading as nothing installed
  - scenario: An operation that reports no changes changes no byte
---

## Requirement

The product MUST generate every provider-specific AI integration — skills, commands and hooks — from a single set of canonical assets. Each generated file MUST carry a managed-file header identifying it as generated and recording version metadata. Generation MUST be reproducible: the same canonical assets, the same target and the same repository configuration produce identical files. The product MUST provide an update command that regenerates the installed integrations, and it MUST detect and report a managed file that has been modified by hand instead of silently overwriting or silently keeping the edit.

Where a rendering choice is offered, it MUST be recorded in the repository configuration rather than supplied per invocation, so that regenerating the integrations cannot silently reverse a decision the repository has already made.

The product MUST remove a managed file that it previously generated and that the current canonical assets and configuration no longer produce. Removal MUST be conditional on the file still matching the content recorded for it: a file that has diverged MUST be left in place and reported, never deleted. Removal MUST be reported alongside what was written.

Installation, regeneration and removal are repository mutations and MUST satisfy `BR-MUTATION-001`. Concretely: the product MUST be able to report what installing or removing an integration would do — create, regenerate, overwrite, remove, preserve or refuse — without writing or deleting anything, and the report MUST agree with what performing it produces, including a refusal over files it will not overwrite. Removing an integration MUST preserve and report every managed file that has diverged from the content recorded for it, and MUST keep that file's record so it remains covered by drift detection; removing a diverged file MUST require an explicit destructive request, which MUST report exactly what it removed. An installation record that exists but cannot be read, parsed or validated MUST stop the operation and be reported, and MUST NOT be treated as an absent record. An operation that reports no changes MUST change no byte: it MUST NOT rewrite a managed file whose content already matches, and MUST NOT rewrite the installation record.

## Rationale

The methodology is delivered to AI assistants through provider-specific files, and each provider wants them in its own shape. If those files were maintained per provider by hand, they would drift — one assistant would follow yesterday's methodology while another follows today's. A single canonical source with generated projections keeps every provider teaching the same rules, and the managed-file header tells both humans and tools which files are projections. Detecting manual edits closes the canonical loop: an improvement made in a generated file would otherwise be lost at the next update, so the product surfaces it and directs the fix to the canonical asset.

Reproducibility is stated over configuration as well as assets because a rendering choice is a real input to the output. Omitting it would make the guarantee false the moment any such choice exists, and a guarantee that is false is worse than a narrower one that holds. Recording the choice in configuration rather than accepting it per invocation is what keeps the guarantee usable: otherwise regeneration would depend on how the command was typed, and drift detection would be comparing against whichever rendering happened last.

Ownership of a generated file is a claim about its absence as well as its presence. A file the product stops generating is dropped from the record of what it owns, and from that moment no integrity check covers it: it is neither regenerated, nor reported as drifted, nor removed. It persists, indistinguishable from something the user wrote, teaching an assistant instructions the repository no longer intends. Leaving it is the more damaging choice. The digest condition is what makes removal safe to state as an obligation — the product removes only what it can prove is its own and untouched, and anything a human has touched stops being the product's to delete.

The mutation obligations restate that same reasoning for the operations that were exempt from it. The digest condition already governed regeneration; applying it to removal is not a new principle but the removal of an inconsistency, because a product that refuses to overwrite a hand-edited file and then deletes it is protecting the file only until the user asks for something else. Keeping the record of a preserved file is what stops the preservation from being a slower form of the same loss: an unrecorded file left on disk is no longer watched by anything.

Reporting without acting is required here for the same reason initialization requires it: the question "what will this do to my repository?" must be answerable without finding out. Requiring the report to include a refusal closes the case the report exists for — a preview that reports success where the real run refuses is wrong exactly when it is consulted. And byte-idempotence is what keeps the report readable over time: a no-op that rewrites files and stamps new timestamps makes every invocation look like a change, which trains a maintainer to stop reading the diff that would have shown a real one.

Failing closed on an unreadable installation record is the difference between an error and a false verdict. An absent record means nothing is installed, which is safe to act on. A record that exists but cannot be trusted means the product does not know what it owns, and reporting that as "nothing installed" produces a clean drift check for an unverifiable installation and then reinstalls over files whose recorded digests were just discarded.

## Acceptance Scenarios

- A repository is initialized with an AI provider selected. The installed skills, commands and hooks each begin with a managed-file header naming the generating product and the asset version.
- The installed integration files are deleted and the update command is run with unchanged canonical assets and configuration. The regenerated files are byte-identical to the originals.
- A user edits one managed file by hand. The doctor and update commands report the modification with its documented diagnostic code and do not silently discard or silently preserve the edit.
- A repository turns off a rendering choice it previously had on, then regenerates. The files that choice produced are removed, their removal is reported, and drift detection is clean afterwards.
- One of those files had been edited by hand. It is left in place and reported rather than removed, and the rest of the removal proceeds.
- A maintainer asks what adding a provider integration would do in a repository that already contains one of its target files with hand-written content. The report names every target by outcome and reports the refusal over the occupied file; the repository is byte-identical afterwards, and the real run refuses in the same way.
- A maintainer removes a provider integration after hand-editing one of its managed files. The edited file is kept, named as preserved, and stays in the installation record; the remaining managed files are removed. Drift detection afterwards still reports the preserved file.
- The same removal is re-run with an explicit destructive request. The edited file is removed, the removal names it, and the installation record no longer carries the integration.
- The installation record is truncated by a bad merge. Health checking and drift checking fail and state that the record cannot be trusted, rather than reporting that no integrations are installed and that all managed files match.
- An integration is added a second time with nothing changed. The command reports no changes, and every file in the repository — the managed files and the installation record alike — is byte-identical and unwritten.
