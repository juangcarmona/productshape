...
The Product Snapshot MUST remain usable as the product model it projects grows well beyond the model the product defines for itself. Scalability is defined over the operations a reader actually performs, on representative models extending to an order of magnitude beyond the current model — never over rendering the whole model simultaneously, which no surface does.
...
**Representative models.** All measurements are taken against a fixed procedure:

1. the current ProductShape model;
2. a synthetic model generated at five times the reference shape;
3. a synthetic model generated at ten times the reference shape.

The measurement harness MUST compile each model and record its exact artifact count, relationship count, maximum degree and isolated-artifact count from the generated graph. Those figures are measurement output, not manually maintained values in this requirement. The synthetic models MUST contain dense relationships, high-degree artifacts, isolated artifacts, and long titles and bodies, so that the hard cases are present rather than assumed away.

Historical and current figures are produced by `pnpm measure:snapshot` and recorded with the evidence for the revision measured. The canonical requirement defines the procedure and thresholds; it does not copy revision-specific model counts that become false after the next Product Change.

**Measures and targets:**

- **Opening document size.** Measure the bytes of markup present in the document when the page opens, for each representative model. Target: from the smallest to the largest representative model — a tenfold increase in the generated reference shape — it grows by less than a factor of two.
...
- The opening document is measured for all three representative models: artifact-level markup, nodes and edges are absent from every one, and its size across a tenfold growth in the generated reference shape grows by less than a factor of two.
...
- All measurements are recorded — model, derived counts, measure, figure, hardware and browser — so any later regression is detectable against the same procedure.
