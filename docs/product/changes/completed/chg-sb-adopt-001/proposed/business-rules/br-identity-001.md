...
Product knowledge outlives any particular file layout. Teams reorganise folders, rename files and retitle artifacts as understanding improves; none of that should break traceability. Anchoring identity in the ID lets every relationship — governed-by, defined-in, derived-from and citations — survive reorganisation untouched, and lets Product Changes refer to artifacts across time without ambiguity. Forbidding ID reuse protects history: a retired ID still means what it always meant, so accepted changes and consumer citations remain intelligible years later.
...
- Reusing a retired ID for a new concept is rejected outright; `SB-ID-REUSE-REJECTED` carries that behaviour, and the new concept receives a fresh ID.
