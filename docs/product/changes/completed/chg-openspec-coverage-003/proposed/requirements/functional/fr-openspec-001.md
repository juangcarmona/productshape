...
verification:
  ...
  - id: S9
    scenario: The product deterministically enumerates every recorded drift warning across the consumer-document population, distinguishing current from archived material and reporting the document, the artifacts involved and the summary, as a report that never gates
---
...
A recorded drift warning MUST carry a machine-readable marker on a line of its own inside the drift note: `<!-- pdac-drift ids="<ID>[, <ID>...]" summary="<one line>" -->`, where `ids` names the product artifacts involved and `summary` states the divergence in one line. The product MUST provide a deterministic way to enumerate every recorded drift warning across the consumer-document population — distinguishing current from archived material — reporting for each the document and location, the artifacts involved with whether each resolves in the current model, and the summary, so a product owner reviews open drift from one listing. Enumeration is a report and MUST NOT gate: it introduces no diagnostic, no exit-code failure for recorded drift, and no judgement of the drift itself.
...
The drift marker exists because the warning serves two readers: the reviewer of one change reads the prose in place, while the product owner sweeping the workspace needs a listing, and a warning that must be hunted for will be missed. Enumeration never gates because a gate would make recording drift punishable, teaching consumers not to record it — visibility, not enforcement, is what the marker buys.
