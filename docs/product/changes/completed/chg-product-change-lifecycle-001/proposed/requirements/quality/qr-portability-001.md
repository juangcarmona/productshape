...
The product MUST behave identically on Linux, Windows and macOS. Every command — including initialization, validation, change validation and apply, graph compilation, impact analysis, citation emission and citation verification — MUST produce the same results, diagnostics and exit codes for the same repository content on all three platforms. Generated files MUST be byte-identical regardless of which platform produced them, independent of native path separators and line-ending conventions.

## Measurement

Conformance is measured by cross-platform comparison: the same repository content is processed on Linux, Windows and macOS, and the outputs are compared. The pass threshold is exact equality — diagnostic lists are identical entry for entry and in order, exit codes match, product diffs and citation statuses match, and generated files such as product graphs, indexes and snapshots are byte-identical across the three platforms, including when the checkout uses CRLF line endings or backslash paths natively. Any divergence is a conformance failure; there is no tolerance band.

## Verification

The product's own automated test suite runs on Linux, Windows and macOS in continuous integration, and the release gate requires all three runs to pass. The suite includes cross-platform fixture tests that validate a shared repository fixture and assert byte-identical generated outputs, identical product diffs, identical citation statuses and identical diagnostics against committed expectations.
