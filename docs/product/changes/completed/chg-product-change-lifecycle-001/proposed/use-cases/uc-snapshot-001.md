...
The Product Engineer runs `prodshape graph --format html`, typically when someone outside the repository workflow needs to understand the product, or after a merged Product Change so a shared snapshot reflects the newly accepted baseline.
