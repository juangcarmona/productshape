...
id: BR-SDD-001
type: business-rule
title: SDD frameworks cite product knowledge but do not own product semantics
status: active
applies-to:
...
- During implementation a developer discovers that two active business rules contradict each other for an edge case. The contradiction becomes input to a Product Change on the product side; the SDD framework never patches the rules itself.
