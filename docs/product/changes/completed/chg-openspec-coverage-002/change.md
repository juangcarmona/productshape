...
Drift must surface — not block, and not be fixed by the tool — because the divergence is a legitimate disagreement between two human-owned statements of intent: the backlog item and the accepted definition. The consumer can detect the disagreement but must not settle it — `BR-SDD-001` already reserves the resolution to humans through the change process. A recorded warning in the proposal is the smallest step that guarantees the disagreement reaches a reviewer; anything less allows silent divergence, anything more has the tool deciding product intent. Deterministic verification stays out of it: drift is a question of meaning no tool can decide, so a flagged divergence is never a conformance criterion and citation verification neither detects nor blocks on it.
