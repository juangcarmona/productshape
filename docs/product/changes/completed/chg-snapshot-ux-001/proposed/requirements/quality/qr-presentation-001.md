...
verification:
  ...
  - scenario: Each artifact kind carries a meaningful inline-SVG icon that aids recognition for readers unfamiliar with the identifier conventions, and the icon accompanies the kind token rather than replacing it
  - scenario: Kind icons are embedded in the file as inline SVG and no icon is fetched from an external resource at open time or afterwards
...
**Kind icons.** Each artifact kind MUST carry a meaningful inline-SVG icon that communicates the kind at a glance to a reader who has not learned the methodology's identifier conventions. The icon MUST accompany the existing kind token and kind label, not replace them: the token is the precise signal for those who know it, and the icon is the unlearned signal for those who do not. Icons MUST be functional — they carry kind meaning — and MUST NOT be decorative illustration. Icons MUST be embedded directly in the snapshot generator source and emitted as inline SVG in the generated file; no icon, icon font, sprite or external asset MAY be fetched at open time or afterwards, consistent with `CON-NO-WEB-UI`. A kind's icon MUST be the same in every view, for every artifact of that kind, and across regenerations, just as its colour is.
...
**Prohibited treatments.** The page MUST NOT use gradients, glass or blur effects, decorative illustration, oversized hero typography, or a generic rounded-card dashboard treatment of its content. Kind icons are functional, not decorative: they are permitted because they carry kind meaning alongside the token, not because they decorate the page.
...
- **Kind icons.** Enumerate the artifact kinds present in the model and confirm each carries an inline-SVG icon in the artifact list, the Reader, the search results and the Focused Topology. Measure: the count of kinds without an icon, plus the count of icons fetched from an external resource rather than embedded in the generator source. Confirm each kind's icon is identical across views and across two regenerations from identical content. Measure: the count of kinds whose icon differs between views or regenerations.
...
- **Prohibited treatments.** Inspect the rendered page for gradients, glass or blur effects, decorative illustration, hero-scale typography, and content enclosed in rounded cards in a dashboard arrangement. Measure: the count of occurrences. Kind icons are excluded from this count when they carry kind meaning alongside the token; they are included when they are purely decorative.
...
- Each artifact kind carries a meaningful inline-SVG icon in every view where the kind appears. The icon is confirmed to be embedded in the generator source and emitted as inline SVG (not fetched), and the same kind's icon is identical across views and across two regenerations from identical content. The kind token and label remain present alongside the icon.
...
- The rendered page is searched for gradients, glass or blur effects, decorative illustration, hero-scale typography and rounded-card dashboard treatment. None is present. Kind icons are present and functional, not decorative.
