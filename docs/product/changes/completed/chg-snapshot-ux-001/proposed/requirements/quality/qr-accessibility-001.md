...
verification:
  ...
  - scenario: Every kind icon has an accessible name stating the artifact kind it represents, so a reader using assistive technology hears the kind rather than encountering an unlabelled image
  ...
  - scenario: Artifact kind remains determinable with the icons removed, because the kind token and label accompany every icon
...
Kind icons MUST carry an accessible name stating the artifact kind they represent, so a reader using a screen reader hears "use case" or "functional requirement" rather than encountering an unlabelled image. The kind token and kind label MUST accompany every icon, so artifact kind remains fully determinable with icons removed: the icon is an accelerator over the same text signal, never a replacement for it.
...
- **Accessible names.** Enumerate controls whose visible content is an icon or symbol. The measure is the count without an accessible name; the target is zero. Enumerate the kind icons present in the artifact list, the Reader, the search results and the Focused Topology. The measure is the count of kind icons without an accessible name stating the kind; the target is zero.
...
- **Icon independence.** Render the page with kind icons removed (e.g., via a stylesheet that hides SVG icons) and confirm artifact kind remains determinable in every view through the kind token and label. The measure is the count of views where kind is no longer determinable; the target is zero.
...
- Every icon-only control is queried for its accessible name and each name states the control's action. Every kind icon is queried for its accessible name and each name states the artifact kind it represents.
...
- With kind icons removed, a reader can still determine each artifact's kind through the kind token and label in every view.
