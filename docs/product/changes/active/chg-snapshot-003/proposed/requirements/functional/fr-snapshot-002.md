...
title: Read one artifact in a persistent inspector beside the map
status: active
derived-from:
  - UC-SNAPSHOT-EXPLORE-001
  - BR-RELATIONSHIPS-001
verification:
  - scenario: The Explorer presents three regions — a left rail for search, bands, filters and browsing; the map in the centre; an inspector on the right
  - scenario: The inspector is always present and scrolls independently of the map and the rail
...
relationships in both directions rather than appearing incomplete. Every artifact MUST display its
status.

On viewports too narrow for three regions side by side, they MUST become distinct navigable states rather
than a scaled-down desktop layout, and the reader MUST be able to move between rail, map and inspector
...
- The snapshot is opened at a viewport width of a common phone. The rail, the map and the inspector are
  usable as separate states; no horizontal scrolling is required to read any of them, and the selected
  artifact survives moving between them.
- A long artifact is selected and the inspector scrolled to its end. The map has not moved and the rail is
  still showing the selection.
- An artifact body containing `<script>alert(1)</script>`, an unclosed `<div`, and an attribute
  fragment such as `" onload="x` is displayed verbatim as text. No script executes and no element
  is created from it, both on first render and after the reader navigates away and back.
- A draft artifact is visibly distinguishable from an active one, and no control on the page
  creates or changes anything. Reloading the page after exploring restores only what the address
  encodes.
