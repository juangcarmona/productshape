...
The canonical AI assets live once, provider-neutrally, in the repository: `skills/`, `commands/` (thin `/product:*` commands) and `templates/`. The push-pipeline hooks were retired by RFC #4. Provider files under `.claude/`, `.github/`, `.agents/` and `.opencode/` are generated from them by the `distribution` package, using the provider mappings exported by `integration-claude`, `integration-copilot`, `integration-codex` and `integration-opencode`.
