...
- `prodshape init --ai claude` adds AI integrations and implies `--full` (the installed skills author from the templates and the per-kind layout). `--ai` accepts a comma-separated list of `claude`, `copilot`, `codex` and `opencode`; integrations can also be added later with `prodshape integration add`.
