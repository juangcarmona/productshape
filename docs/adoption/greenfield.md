...
> These commands target the supported published baseline, [`@prodshape/cli@0.19.0-alpha.0`](https://www.npmjs.com/package/@prodshape/cli/v/0.19.0-alpha.0). `prodshape` is canonical; `product-definition` is an identical v0.x compatibility alias, removed before v1. Every step marked "by hand" also works without the CLI: the layout, schemas and templates are plain files. See [Limitations](../limitations.md).
