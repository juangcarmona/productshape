...
Features that predate the adoption fail this gate as `unclassified` until they are bound, by design. The `bind-consumers` skill (`/product:bind`) drives that backfill: per document it finds the governing artifacts, inserts the citations, declares scope, records drift where the feature contradicts the model, and proposes exemptions for your approval; done means the command above exits clean.

If you place a feature outside `specs/` by giving Spec Kit an explicit `SPECIFY_FEATURE_DIRECTORY`, verification also enumerates the directory it recorded in `.specify/feature.json`, so that feature is gated rather than silently skipped. Spec Kit records one feature there, the latest, so keep features under `specs/` if you want the whole population enumerated: an out-of-tree feature from an earlier run leaves no record to find.
