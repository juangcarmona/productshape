...
> Supported published baseline: `@prodshape/cli@0.14.0`. This file is deliberately unversioned: a limitations document named after one release stops describing the product at the next one. Items explicitly labelled **Unreleased** describe the next release candidate on `main`, not the published baseline.
