...
- Verify the slice is approved and `prodshape change validate <CHG-ID>` is clean.
- Generate with `prodshape handoff create --change <CHG> --slice <SLI>
--work-item <provider:owner/repo#id> --adapter openspec --sdd-change <name>` (or `--out <dir>`).
- Verify with `prodshape handoff status <path>`; report the change's open questions to
  the SDD side.
