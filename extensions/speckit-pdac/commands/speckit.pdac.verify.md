...
2. Read the result. Every feature's `spec.md`, `plan.md` and `tasks.md` gets one scope state, and every citation one status. Act on each finding:
   - `unclassified` document (PRODUCT064): the document must end up bound or exempt, and binding takes two things, not one. Cite the canonical artifacts it derives from (`npx prodshape inspect <ID>` for the digest, then `npx prodshape cite --id <ID> --digest <digest>`, one citation line under the text it grounds), and add a scope declaration: an HTML comment of its own carrying a `pdac-scope` key set to the value `cited`. Citations alone never bind, and the declaration alone never binds either; a declaration is read only from that HTML comment or from a `pdac-scope` frontmatter key, and Spec Kit documents have no frontmatter. The exact form is in `.specify/memory/pdac.md`. Exemption is a human decision: ask the user before declaring it, and never declare it just because citations are missing.
