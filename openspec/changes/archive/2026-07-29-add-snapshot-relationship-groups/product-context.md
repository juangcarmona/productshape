<!-- GENERATED by prodshape handoff create for HOF-GITHUB-31. Non-canonical. -->
<!-- Regenerate with: prodshape handoff create. Do not edit by hand. -->

# Product context — HOF-GITHUB-31

Work item: `github:juangcarmona/productshape#31` — SLI-SNAPSHOT-004: Typed relationships in both directions, without a graph

Source: revision `ab8adff3af9a27969f09f4c6b455847c056de03d` (CHG-SNAPSHOT-002 / SLI-SNAPSHOT-004)

## Delivery outcome

A Product Explorer reading one artifact understands what it connects to and what connects to it — what the artifact itself declares, kept separate from the derived reverse references no authored file states — with each relationship naming its type and direction, each related artifact selectable in one step, high-degree groups collapsed with exact counts until asked to open, and artifacts holding no relationships saying so rather than appearing broken. All of it as text: no visualization is required to understand any relationship in the model.

Verification:

- From a use case, the reader sees the governing business rule in the declared group and the functional requirement that derives from it in the derived group, and reaches either in one step without needing to know which side authored the edge.
- Every relationship group entry names its relationship type and its direction explicitly, and both remain determinable with colour removed.
- For every artifact in the model, the declared and derived groups together account for exactly the edges `prodshape graph` records for it — no edge missing, none invented, and no importance, health, ownership or ordering implied.
- Selecting the most connected artifact in the current baseline (27 relationships) shows neighbours grouped by relationship type and artifact kind, with groups large enough to overwhelm the view collapsed and showing their exact counts; expanding one reveals exactly that number, and nothing expands until the reader expands it.
- The same holds for the highest-degree artifact of the largest representative model (171 relationships), and the page stays responsive while a group is expanded.
- Selecting an artifact with no relationships — `CON-NO-WEB-UI` in the current baseline — reports the absence in both directions and reads normally, at every measured scale.
- Related-artifact titles and relationship labels rendered from embedded data are escaped — a title containing a script tag or an attribute-injection fragment is displayed verbatim as text with nothing executed.
- Following a relationship makes the target the page's single selected artifact and updates the address through the same navigation mechanism; following three in sequence and pressing Back three times returns through them in reverse order, and Forward retraces them.
- Expanding a group, collapsing it and following a relationship are all operable by keyboard alone, with visible focus, deliberate focus placement after the selection changes, and expanded state exposed as state; icon-only expansion controls report accessible names; with a reduced-motion preference set, expansion runs without animation.
- The relationship groups adapt on a narrow viewport without horizontal page scrolling, and every text-and-background pair introduced here passes WCAG 2.1 AA.
- Recorded screenshots at both viewports, for both representative models, confirm the relationship groups follow the presentation system established in SLI-SNAPSHOT-003 — same accent, same per-kind palette, same borders and alignment — rather than introducing a second visual language.
- No server, runtime process, network request or external resource is involved; expansion state is not persisted outside the address; browser storage and cookies remain empty.
- Regenerating from identical model content still produces a byte-identical file; the opening document still contains no artifact body; and artifact-selection latency is re-measured on the reference environment and still meets the budget agreed in SLI-SNAPSHOT-003.

Out of scope:

- Any visual rendering of relationships; this slice delivers the textual equivalent that SLI-SNAPSHOT-006's focused neighbourhood projection is required to have.
- Ranked search; SLI-SNAPSHOT-005 delivers it.
- Relationships beyond one hop from the selected artifact; deeper reachability belongs to the impact-analysis capability, not the snapshot.
- Changing which relationships exist or how they are derived; the groups render the compiled graph as it is, and reverse relationships remain derived rather than authored.
