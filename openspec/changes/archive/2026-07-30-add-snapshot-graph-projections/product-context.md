<!-- GENERATED by prodshape handoff create for HOF-GITHUB-35. Non-canonical. -->
<!-- Regenerate with: prodshape handoff create. Do not edit by hand. -->

# Product context — HOF-GITHUB-35

Work item: `github:juangcarmona/productshape#35` — SLI-SNAPSHOT-006: Focused neighbourhood and the layered model map

Source: revision `d313e9e84c4e729b1e00d4f33b9a83a261e043b4` (CHG-SNAPSHOT-002 / SLI-SNAPSHOT-006)

## Delivery outcome

A Product Explorer can see what they have been reading. The focused neighbourhood draws the selected artifact's one-hop relationships with incoming told apart from outgoing and every edge naming its type, and the layered model map places real artifacts in four fixed bands as the snapshot's broad-topology view — filtering, grouping and collapsing to stay legible, and stating exactly how much it is holding back. Selecting a node anywhere moves the same single selection, and with the final surface in place the whole exploration progression is confirmed coherent end to end. No view draws the whole graph at once.
...
- Selecting a use case and opening the focused projection shows an unmistakable anchor, incoming and outgoing relationships told apart without relying on colour, and each edge naming its relationship type; one hop is shown by default.
- The layered map for the current baseline places actors and bounded contexts in Product context, journeys and use cases in Product behaviour, business rules and domain terms in Rules and language, and functional requirements, quality requirements and constraints in Product commitments.
- The same band assignment appears in a snapshot generated from an unrelated product model with different kind proportions, with nothing in either file or its configuration able to change it.
- Nothing in the layered map states or implies that one band precedes, causes, depends on or supersedes another. A relationship from a functional requirement to a use case and one from a use case to a domain term both display their authored direction regardless of band order.
- The layered map opened for the largest representative model does not render every node and edge; it offers filtering and grouping and states the exact number of artifacts and relationships collapsed or hidden, and expanding a group reveals exactly that number.
- A snapshot generated from a model with no bounded contexts renders only the bands the model populates, with no empty or invented band.
- Selecting a node in either projection makes that artifact the page's single selection and updates the address including the active graph mode; a projection view is directly linkable; leaving the projection keeps the artifact selected; Back and Forward restore both the selection and the projection view.
- In one pass, an artifact selected from the list, then found through search, then reached through a relationship link, then selected as a node in a projection all produce the same single selection, with the selected state visible wherever the artifact appears.
- After a node selection in either projection changes the artifact, exactly one artifact detail is active, inspecting the document confirms no other artifact's body is present, and the master area's selected state has followed the change and persists as the reader continues working.
- No projection surface offers any capability to create, edit, annotate, approve or persist anything.
- Hiding or disabling the visual projection leaves every incoming and outgoing relationship of the selected artifact readable with type and direction through the SLI-SNAPSHOT-004 list.
- Both projections are fully operable by keyboard, including node selection, filtering, collapsing, expanding and fitting; icon-only controls report accessible names; expanded, collapsed and selected state is exposed as state; nothing on a node or edge is hover-only; with reduced motion set, no non-essential transition runs.
- The complete exploration progression is completed in one pass using only the keyboard, with visible focus at every stop and no focus lost across any view or selection change; landmarks and the heading outline are reviewed across all views together with no region unlabelled and no level skipped.
- Every text-and-background pair the finished stylesheet can produce is computed against WCAG 2.1 AA and passes, including the kind palette applied to text and to node labels; with colour removed, artifact kind, relationship direction, artifact status and selection remain determinable on every view.
- The finished page uses one accent colour, and each artifact kind's colour is identical in the orientation view, the artifact list, the artifact detail, the relationship groups, the search results and both projections — and identical again in a second snapshot generated from the same model content. Screenshots at both viewports, for both representative models, are recorded.
- Two snapshots generated from identical model content on different platforms produce identical projection arrangements, and the file remains byte-identical.
- Every node, relationship and direction either projection draws is confirmed present in `prodshape graph` output, nothing else is drawn, and no importance, health, ownership or ordering is implied.
- Focused-projection latency is measured for all three representative models on the reference environment, the figures recorded in full, confirmed not to degrade materially with model size, and the focused-projection budget derived from them and agreed before this slice's implementation is approved.
- No server, runtime process, network request or external resource is involved at any point in a full exploration pass; filter, collapse, zoom and fit state is not persisted; browser storage and cookies remain empty; and the file is still exactly one self-contained HTML document working over `file://` with networking disabled and from ordinary static hosting.

Out of scope:

- Any unstructured whole-model graph view; removed from this change by product decision, with the layered model map taking the broad-topology role.
- Multi-hop neighbourhood expansion beyond one hop by default; deeper reachability belongs to the impact-analysis capability.
- Editing the graph, repositioning nodes persistently, or any layout the reader can save; the snapshot persists nothing beyond the address.
- Adopter-configurable or per-model band assignment; the bands are fixed and product-owned, and a new artifact kind receives its band through a Product Change.
- Latency budgets for interactions this slice does not introduce; artifact selection belongs to SLI-SNAPSHOT-003 and search to SLI-SNAPSHOT-005.
- The artifact list, artifact body and metadata rendering, their escaping, the relationship groups and the responsive master-detail states; this slice carries only the graph portion of the master-detail invariants, delivered against the surfaces the earlier slices established.
...
5. **What is the interaction form of the focused neighbourhood? — Resolved: an orbit of groups.** The product owner asked to see the alternatives and chose this one after comparing three ASCII mockups, each drawn against both a typical artifact (8 relationships) and the hardest one in the model (`BC-PRODUCT-DEFINITION`, 27, all incoming).
   ...
   The selected artifact anchors the centre. What orbits it are the **relationship groups**, not the individual artifacts: each satellite states its relationship type, the artifact kind at the other end, and its exact count. Incoming and outgoing occupy opposite sides of the anchor's axis, so direction is carried by position and survives with colour removed. Opening a satellite fans its members into a local arc; the other satellites stay where they are rather than the view re-anchoring, which keeps the arrangement calm and stable as the reader opens and closes groups. Groups small enough to read at a glance arrive already open, so a typical artifact needs no clicking at all.
