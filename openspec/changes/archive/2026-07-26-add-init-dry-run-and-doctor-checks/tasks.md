...
- [x] 1.1 Split `installProvider` into `planProvider` and `applyProviderPlan`, keeping the lock read
      at apply time so installing a second provider does not drop the first
...
- [x] 1.7 Add the injected `model validation` doctor check, using `validateBaseline` so `doctor`
      writes nothing
- [x] 1.8 Add the three-state `authoring templates` doctor check
- [x] 1.9 Recommend `init --dry-run` in the brownfield guide and correct the wrong claims about what
      `init` touches
