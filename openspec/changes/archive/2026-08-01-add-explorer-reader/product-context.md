<!-- GENERATED by prodshape handoff create for HOF-GITHUB-47. Non-canonical. -->
<!-- Regenerate with: prodshape handoff create. Do not edit by hand. -->

# Product context — HOF-GITHUB-47

Work item: `github:juangcarmona/productshape#47` — SLI-EXPLORER-002: Read an artifact and every canonical relationship it holds

Source: revision `38bf111f1cbb742176ef0629579fab8c3d45d498` (CHG-SNAPSHOT-004 / SLI-EXPLORER-002)

## Delivery outcome

The selected artifact dominates the Reader: identity, type, status, metadata and the author's canonical content, with every canonical relationship grouped by its actual meaning, direction distinguished where it matters, complete counts on every group, and every related artifact one step from becoming the new focus. Deep links, Back/Forward and preserved navigation context make the model navigable as a graph through reading alone — no node-link diagram required.

Verification:

- A selected artifact shows its identity, type, status, remaining metadata and authored Markdown with the author's heading hierarchy; no other artifact's body is present in the document.
- Declared and derived relationships appear as separately labelled groups by relationship meaning, direction explicit where it matters, each group carrying its complete count, each entry naming the related artifact's title and identifier and selectable in one step.
- An isolated artifact reports both directions empty and reads normally; the most connected artifact's groups are counted rather than spilled.
- Following a relationship makes the target the new focus everywhere; Back returns to the previous artifact with the reader's prior context — including an in-progress Catalog discovery — intact.
- Every relationship of the compiled graph is reachable by reading alone: walking the Reader's groups accounts for every edge that touches each inspected artifact.
- Selection, reading and traversal latency are measured at the three representative scales and recorded; nothing degrades materially across the range.
- The reader's navigation context is visible and retraceable from the Reader: arriving from a Catalog discovery, the way back is apparent without relying on browser chrome alone, and Back restores it.
- The whole reading flow is keyboard-operable with visible focus; authored content containing markup displays as text and never executes; regeneration from identical content remains byte-identical and browser storage stays empty.

Out of scope:

- The Focused Topology; SLI-EXPLORER-003.
- Any behaviour beyond FR-SNAPSHOT-002's proposed contract; the Reader is delivered exactly as the change defines it.
...
- FR-SNAPSHOT-002: `docs/product/changes/active/chg-snapshot-004/proposed/requirements/functional/fr-snapshot-002.md` (sha256:b406ace18e4e48701bde721e6f86f6a12730c4c36c7aa6bc5c931717bd9987b7)
