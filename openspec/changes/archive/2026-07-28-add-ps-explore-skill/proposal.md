...
ProductShape's change workflow begins at `ps:change`, which requires the user to already know
"what should be different and why." Product owners and developers often start with a fuzzy idea
that needs to be challenged, refined, and grounded in the existing product model before it can
become a well-formed change request. There is no guided entry point for that pre-proposal
thinking — this gap makes the workflow feel abrupt and produces weaker change requests.

## What Changes

- Introduce a new `ps:explore` skill that acts as a product-graph-aware thinking partner before
  `ps:change` is invoked.
- The skill reads the full product model upfront and reasons from a high-altitude structural view,
...
- `product-exploration`: The `ps:explore` skill — loading the product graph, running structural
  analysis, conducting a guided idea-refinement conversation, and handing off to `ps:change`.
